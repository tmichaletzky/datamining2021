...
def Embeddings(df, model="tfidf", columns=["text", "label"], drop_neutral=False, config=None):
    print("Preparing %s embedding"%model)
    if drop_neutral:
        print("\tdropping neutral sentiments")
        df = df[df[columns[1]] != score2label(0)]
...
def create_heatmap(metrics_df, embedding, part="test", split="metric", values="score", fsize=(8,4)):
    tmp_df = metrics_df[metrics_df["part"] == "test"]
    pivot_df = tmp_df.pivot(index="model", columns=split, values=values)    
